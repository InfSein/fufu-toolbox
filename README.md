...
## 项目简介
Fufu Toolbox 是一个可持续扩展的桌面小工具集合。现在已接入四个工具：合并TXT、合并PDF、FFXIV物品翻译、端口管理。
...
- 物品翻译：`XivItemTranslationService` 负责调用接口翻译 FFXIV 物品名
- 端口管理：`PortManagementService` 负责读取端口占用、筛选并中止进程
...
- 接入“FFXIV物品翻译”：
  ...
  - 显示翻译进度，失败项保留原文并给出原因
- 接入“端口管理”：
  - 展示本机当前端口占用（TCP/UDP、端口号、PID、程序名、程序图标/路径）
  - 支持按端口号或程序名筛选
  - 中止程序前弹确认框
  - 权限不足时尝试触发管理员提权中止
- 移除“关于”页面
