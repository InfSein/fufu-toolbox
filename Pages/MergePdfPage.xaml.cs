...
{
    public MergePdfViewModel ViewModel { get; }

    public MergePdfPage()
    {
        ViewModel = App.GetService<MergePdfViewModel>();
        InitializeComponent();
    }
    ...
    {
        if (sender is Button { Tag: MergePdfFileItem item })
        {
    ...
    private async void MergeAndSaveButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            await ShowInfoAsync("无法合并", "请至少选择一个文件。");
            return;
        }
        ...
        picker.FileTypeChoices.Add("PDF 文件", new List<string> { ".pdf" });
        InitializeWithWindow.Initialize(picker, GetMainWindowHandle());
        ...
        try
        {
            await ViewModel.MergeSelectedFilesAsync(outputFile.Path);
